...
export default function Collections() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const empData = snapEmp.docs.map(doc => ({ id: doc.id, ...doc.data() } as Employee));
  ...
  const handleOpenModal = (coll?: CollectionData) => {
    if (coll) {
      ...
      const selectableCollectors = employees.filter(e => ['cobrador', 'ambos'].includes(e.role));
      setEditingCollection(null);
      setFormData({
        employeeId: selectableCollectors[0]?.id || '',
        initialDate: format(new Date(), 'yyyy-MM-dd'),
        finalDate: format(new Date(), 'yyyy-MM-dd'),
        noReceipt: false,
        initialReceipt: '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Auto switch current month if user registered an anterior/different date
      const targetMonth = formData.initialDate ? formData.initialDate.substring(0, 7) : currentMonth;
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {paginatedCollectors.map(collector => {
            const isSupervisor = collector.role === 'supervisor_cobranza' || collector.role === 'supervisor_general';
            const collectorCollections = isSupervisor
              ? collections
              : collections.filter(c => c.employeeId === collector.id);
              ...
              <div key={collector.id} className="bg-white dark:bg-neutral-900 rounded-2xl border border-neutral-200 dark:border-neutral-800 overflow-hidden shadow-sm">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        ...
                        {isSupervisor && (
                          <span className="text-[10px] font-bold bg-indigo-100 text-indigo-800 dark:bg-indigo-900/50 dark:text-indigo-300 px-2 py-0.5 rounded-full">
                            Sumatoria Consolidada
                          </span>
                        )}
                      </div>
                    </div>
                ...
                {isExpanded && (
                  <div className="border-t border-neutral-200 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-900/50">
                    {isSupervisor && (
                      <div className="px-6 py-3 bg-indigo-50/80 dark:bg-indigo-950/40 border-b border-indigo-100 dark:border-indigo-900/30 flex items-center gap-2 text-xs text-indigo-900 dark:text-indigo-200 font-medium">
                        <Receipt className="w-4 h-4 text-indigo-600 dark:text-indigo-400 shrink-0" />
                        <span>Vista de supervisor: Refleja la sumatoria de todas las cobranzas registradas por los cobradores del mes.</span>
                      </div>
                    )}
                        ...
                        <div className="overflow-x-auto">
                          <table className="w-full text-sm text-left whitespace-nowrap">
                            <thead className="bg-neutral-100/50 dark:bg-neutral-800/30 text-neutral-600 dark:text-neutral-400 font-medium">
                              <tr>
                                {isSupervisor && <th className="px-6 py-4">Cobrador</th>}
                            ...
                            <tbody className="divide-y divide-neutral-200 dark:divide-neutral-800">
                              {paginatedCollections.map((coll) => {
                                const collEmp = employees.find(e => e.id === coll.employeeId);
                                return (
                                  <tr key={coll.id} className="hover:bg-white dark:hover:bg-neutral-800/50 transition-colors">
                                    {isSupervisor && (
                                      <td className="px-6 py-4 font-semibold text-neutral-800 dark:text-neutral-200">
                                        {collEmp ? `${collEmp.name} ${collEmp.lastName}` : 'Cobrador'}
                                      </td>
                                    )}
          ...
          <div className="bg-white dark:bg-neutral-900 rounded-2xl w-full max-w-2xl shadow-xl border border-neutral-200 dark:border-neutral-800 overflow-hidden animate-in zoom-in-95 duration-200 my-8">
            <div className="flex items-center justify-between p-6 border-b border-neutral-200 dark:border-neutral-800">
              <h2 className="text-xl font-bold text-neutral-900 dark:text-white flex items-center gap-2">
                ...
                <span>{editingCollection ? 'Editar Cobranza' : 'Registrar Cobranza'}</span>
              </h2>
              <button 
                type="button"
